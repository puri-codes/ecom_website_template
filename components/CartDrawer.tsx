    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-16">
        <div className="w-screen max-w-lg bg-[#F9F8F2] shadow-2xl flex flex-col justify-between">
          ...
          {/* Cart Header */}
          <div className="px-8 py-6 border-b border-[#765439]/20 flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8">
            {items.length === 0 ? (
              ...
              items.map((item, idx) => (
                <div key={`${item.product.id}-${item.size}-${item.color}-${idx}`} className="flex space-x-5 border-b border-[#765439]/15 pb-8">
                    ...
                    <div>
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          ...
                          className="text-[#765439] hover:text-[#121212] transition-colors p-1"
                          title="Remove item"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <p className="text-xs font-mono-code text-[#765439] mt-1.5">
                        Size: {item.size} · Color: {item.color}
                      </p>
                    </div>

                    <div className="flex justify-between items-center mt-4">
                      <div className="flex items-center border border-[#765439]/30 rounded-xs">
                        <button
          ...
          {/* Cart Footer */}
          {items.length > 0 && (
            <div className="px-8 py-8 border-t border-[#765439]/20 bg-[#F5F3ED] space-y-5">
              <div className="space-y-2 text-sm font-mono-code">
                ...
                <p className="text-xs text-[#765439]">
                  {subtotal >= 15000 
              ...
              <button
                ...
                className="w-full py-4 bg-[#492d22] text-[#e9eae2] text-sm font-mono-code uppercase tracking-widest hover:bg-[#cec3b1]/85 hover:text-[#492d22] transition-colors rounded-xs font-medium text-center block"
              >
                Proceed to Checkout
              </button>
              ...
              <button
                onClick={clearCart}
                className="w-full text-center text-xs font-mono-code text-[#765439] hover:text-[#121212] transition-colors"
              >
                Clear Cart
              </button>
            </div>
